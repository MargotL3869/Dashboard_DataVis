...
layout = html.Div([
    ...
    dbc.Row([
        ...
        # Page 2
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("2. Comparaison Villes", className="text-primary"),
            html.P("Comment notre ville se situe-t-elle par rapport aux autres villes française de notre région ?")
        ]), className="h-100"), width=4),

        # Page 3
        dbc.Col(dbc.Card(dbc.CardBody([
            html.H5("3. International", className="text-primary"),
            html.P("Globale : Mise en regard des trajectoires climatiques françaises avec les grandes puissances mondiales.")
        ]), className="h-100"), width=4),
    ], className="mb-3"),

    html.Div([
        html.Small("Note technique : Les données internationales sont basées sur les moyennes annuelles de température terrestre par pays.")
